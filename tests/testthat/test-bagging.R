...
test_that("Bagging initializes correctly", {
    ...
    testthat::expect_s7_class(bag, Bagging)
    testthat::expect_equal(bag@ref$B, 5L)
    testthat::expect_equal(bag@ref$type, "regression")
    testthat::expect_length(bag@ref$trees, 5L)
})

# fit_bag
test_that("fit_bag errors when X is not matrix", {
    bag <- make_bag_regression()
    testthat::expect_error(fit_bag(bag, c(1,2,3), c(1.0, 2.0, 3.0)))
})
test_that("fit_bag errors when X and y have differenet lengths", {
    bag <- make_bag_regression()
    testthat::expect_error(fit_bag(bag, make_X(), c(1.0, 2.0)))
})
test_that("fit_bag builds B trees", {
    set.seed(42)
    bag <- make_bag_regression()
    fit_bag(bag, make_X(), make_y_reg())
    testthat::expect_equal(length(bag@ref$trees), 5L)
    testthat::expect_true(all(vapply(bag@ref$trees, function(t) S7::S7_inherits(t, CART), logical(1))))
})

# predict_bag
test_that("predict_bag errors on unfitted ensemble", {
  bag <- make_bag_regression()
  testthat::expect_error(predict_bag(bag, c(0.1, 0.2)))
})
...
test_that("get_tree returns correct tree", {
    ...
    testthat::expect_s7_class(tree, CART)
})
...
test_that("get_tree errors on out-of-range index", {
  ...
  bag <- make_bag_regression()
  fit_bag(bag, make_X(), make_y_reg())
  testthat::expect_error(get_tree(bag, 0L))
  testthat::expect_error(get_tree(bag, 6L))
})
